...
import { useUpdateCase } from "@/hooks/features/use-cases";
...
type EditCaseValues = z.infer<typeof caseSchema>;

const emptyParty: EditCaseValues["parties"][number] = {
...
): EditCaseValues["parties"] {
  return parties.map((p) => ({
    partyType: p.partyType as EditCaseValues["parties"][number]["partyType"],
    entityType: p.entityType as EditCaseValues["parties"][number]["entityType"],
...
export function EditCaseModal({
  ...
  const form = useForm<EditCaseValues>({
    ...
    defaultValues: {
      ...
      practiceArea: caseDetail?.practiceArea || "",
      jurisdictionCode: caseDetail?.jurisdictionCode || "",
      priority: (caseDetail?.priority || "NORMAL") as any,
      parties:
        (caseDetail?.parties?.length || 0) > 0
          ? mapPartiesToFormValues(caseDetail.parties)
  ...
  const onSubmit = async (values: EditCaseValues) => {
    const payload = {
      ...values,
