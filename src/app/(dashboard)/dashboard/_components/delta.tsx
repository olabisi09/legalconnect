...
function DeltaIcon({
  ...
  className,
  ...props
}: Omit<React.ComponentProps<RemixiconComponentType>, "fill"> & {
  className?: string;
  ...
  if (!resolvedValue || resolvedValue === 0) {
    return shell(
      <RiSubtractLine {...slotProps} className={mergedClassName} {...props} />,
    );
  }
  ...
  if (resolvedValue > 0) {
    if (variant === "trend") {
      return shell(
        <RiArrowUpLine {...slotProps} className={mergedClassName} {...props} />,
      );
    }
  ...
  if (variant === "trend") {
    return shell(
      <RiArrowDownLine {...slotProps} className={mergedClassName} {...props} />,
    );
  }
